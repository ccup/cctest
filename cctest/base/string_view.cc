...
bool StringView::ends(StringView suffix) const {
  return suffix.empty()
      || (size() >= suffix.size()
          && memcmp(data() + (size() - suffix.size()), suffix.data(),
              suffix.size()) == 0);
}
...
std::vector<std::string> StringView::safeSplit(StringView delims) const {
...
std::vector<std::string> StringView::split(StringView delims) const {
  return !empty() ? safeSplit(delims) : std::vector<std::string>();
}
